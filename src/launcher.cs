...
static class Launcher
{
        ...
        if (!File.Exists(configPath))
        {
            ...
            if (File.Exists(backupPath))
            {
                ...
                if (!string.IsNullOrEmpty(backupKey))
                {
                    ...
                    string providerId = "opencode"; // Default provider
                    string json = "{\n  \"$schema\": \"https://opencode.ai/config.json\",\n  \"provider\": {\n    \"" + providerId + "\": {\n      \"options\": {\n        \"apiKey\": \"" + backupKey.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"\n      }\n    }\n  }\n}";
        ...
        if (!hasKey)
        {
            ...
            if (File.Exists(backupPath))
            {
                ...
                if (!string.IsNullOrEmpty(backupKey))
                {
                    ...
                    {
                        // Try to find existing provider ID
    ...
    static void SaveApiKey(string configPath, int providerIndex, string apiKey)
    {
        ...
        else
        {
            ...
            {
                // Add to existing provider section
        ...
        // Make config file read-only to prevent accidental deletion
        File.SetAttributes(configPath, File.GetAttributes(configPath) | FileAttributes.ReadOnly);

        // Also save a backup of the key
